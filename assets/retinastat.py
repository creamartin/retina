...
def st(auto,man,layer):
    for x in range(511,0,-1):
        #print("Auto:", auto[x] ,"Manuell:" ,man[511-x])
        dev=abs(1-(auto[x]/man[511-x]))*100        
        if(np.isfinite(dev) and dev <100):
            dev_array[511-x]=dev
        num_dev=abs(auto[x]-man[511-x])
        if(np.isfinite(num_dev) and num_dev <100):
            num_dev_array[511-x]=num_dev        
