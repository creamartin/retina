...
def example(original, manual):
    ...
    flattened, invert = segmentation_helper.flatten(resized)
    ...
    ax.set_axis_off()
    fig.add_axes(ax)
    ax.imshow(np.hstack((resized, np.zeros_like(resized), unflatten)), cmap='gray', alpha=1)
    manual = cv2.resize(manual, (paths.shape[1], paths.shape[0]))
    manual = cv2.cvtColor(manual, cv2.COLOR_RGB2RGBA)
    ax.imshow(np.hstack((np.zeros_like(paths), manual, paths)))
    plt.setp(ax, xticks=[], yticks=[])
    plt.title("Original                     Manual                  Automated ",fontsize=16)
    plt.show()


# Example
normal = cv2.imread('../testvectors/healthy/normal/38B062A0.tif', 0)
normal_segmented = cv2.imread('../testvectors/healthy/normal/38B062A0_RS.tif')

vessels = cv2.imread('../testvectors/sick/wide area of disruption/7261CED0.tif', 0)
vessels_segmented = cv2.imread('../testvectors/sick/wide area of disruption/7261CED0_RS.tif')

drusen1 = cv2.imread('../testvectors/sick/with druses/61D70FB0.tif', 0)
drusen1_segmented = cv2.imread('../testvectors/sick/with druses/61D70FB0_RS.tif')

drusen2 = cv2.imread('../testvectors/sick/with druses/7CE97D80.tif', 0)
drusen2_segmented = cv2.imread('../testvectors/sick/with druses/7CE97D80_RS.tif')

#example(normal,normal_segmented)
#example(vessels,vessels_segmented)
#example(drusen1, drusen1_segmented)
example(drusen2, drusen2_segmented)
